...
**Note:** VMark uses Mermaid v11 (strict Langium parser). Prefer `flowchart` over `graph`. Quote node labels with special characters: `["Label (detail)"]`. No trailing semicolons. If you have the `mermaid-validator` MCP server installed, validate diagrams before inserting.
...
**Note:** Markmap renders heading-based markdown as interactive mindmap trees. Use `#` headings to define the tree hierarchy. Supports markdown formatting within nodes (bold, italic, code, links).
...
Use when `document_replace` returns 0 matches because text spans formatting boundaries.

**Call (cross-formatting text):**
```json
{
  "tool": "document_replace_in_source",
...
{
  "count": 2,
  "message": "Replaced 2 occurrences in source",
  "suggestionIds": ["sug_a1", "sug_a2"],
  "applied": false
}
```

**Note:** Search string must match raw markdown — include `**`, `_`, `[]()` syntax markers.

### workspace_reload_document

Reload document from disk after external edits.

**Call:**
```json
{
  "tool": "workspace_reload_document"
}
```

**Response:**
```
"Reloaded document from: /Users/writer/Documents/paper.md"
```

**Call (force — discard unsaved changes):**
```json
{
  "tool": "workspace_reload_document",
  "force": true
